...
async function downloadGame(id, e) {
    ...
    try {
        // 1. Get Game Metadata from your Server
        const res = await fetch(`${GAME_LIBRARY_SERVER}/games/${id}`);
        const data = await res.json();
        if (!data.ok) throw new Error("Game metadata not found");
        
        const game = data.game;
        const filename = `global_${id}.swf`; // Unique name for caching
        ...
        // 2. CHECK CACHE (Using your existing app.js logic)
        if (isCached && window.Platform?.isCordova) {
            log(`Cache Hit: ${game.title}`);
            try {
        ...
        if (!bytes) {
            log(`Downloading ${game.title} from Global Server...`);
            updateLoader("Downloading...", 20);

            // Fetch binary from your new /games/file/:id route
            const fileRes = await fetch(`${GAME_LIBRARY_SERVER}/games/file/${id}`);
            if (!fileRes.ok) throw new Error("Server failed to provide file");
            
            const buffer = await fileRes.arrayBuffer();
            bytes = new Uint8Array(buffer);

            // 4. SAVE TO DEVICE (Cordova only)
            if (window.Platform?.isCordova && bytes) {
                try {
                    await saveToDevice(filename, bytes);
                    localStorage.setItem(cacheKey, 'true');
                    log(`Saved to persistent storage: ${filename}`);
                } catch (e) {
                    log("Failed to save to device: " + e.message);
                }
            }
        }

        // 5. RUN THE GAME
        updateLoader("Starting...", 80);
        ...
        btn.disabled = false;
        log(`Playing Global Game: ${game.title}`);

    } catch (err) {
        console.error(err);
        btn.innerText = '❌';
        setTimeout(() => { btn.innerText = '▶ Play'; btn.disabled = false; }, 3000);
